...
// Instruction memory RAM. Bootloaders can write this region through Wishbone;
// firmware-only simulations may initialize it directly from INIT_FILE.
module imem_wb #(
    parameter integer IMEM_BYTES = 16*1024,
    parameter INIT_FILE = ""
)(
    input  wire        wb_clk_i,
    input  wire        wb_rst_i,
    ...
    localparam integer WORDS = IMEM_BYTES / 4;
    ...
    initial begin
        for (k = 0; k < WORDS; k = k + 1)
            ram[k] = 32'h0000_0013; // RISC-V NOP
        if (INIT_FILE != "")
